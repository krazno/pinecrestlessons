...
(function () {
  ...
  if (ACTIVITY_SUBMIT.formspreeId && submitMeta) {
    submitMeta.textContent = 'turned in online';
  } else if (submitMeta) {
  ...
  function syncToFormspree(payload) {
    ...
    fd.append('_subject', payload.studentName + ' · ' + activityLayerLabel(payload.level) + ' · Pinecrest CS');
    ...
    return fetch('https://formspree.io/f/' + ACTIVITY_SUBMIT.formspreeId, {
      ...
      if (!res.ok) return res.json().then(function (j) { throw new Error(j.error || 'Could not turn in. Try again.'); });
      return true;
    });
  }
  ...
  submitBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      const msg = synced
        ? 'Turned in! Nice work.'
        : 'Saved on this device. Online turn-in is not set up yet.';
      setStatus(msg, 'ok');
      clearForm();
      await renderHistory();
    } catch (err) {
